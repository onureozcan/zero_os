...
void *k_malloc(size_t size) {
    console_trace(LOG_TAG, "malloc %d bytes\n", size);
    size = ALIGN_4(size);
    struct k_malloc_node *current = head;
    while (current != NULL) {
        console_trace(LOG_TAG, "current:%p is_free:%d, size:%d\n", current, current->is_free, current->size);
    ...
    console_error(LOG_TAG, "unable to deliver %d bytes requested\n", size);
